...
public class ShadowSheath
{
    ...
    public static IEnumerable<Feat> GetFeat()
    {
        ...
        ikon.IkonFeat.WithPermanentQEffect(null, q =>
        {
            ...
            q.ProvideSectionIntoSubmenu = (q, submenu) =>
            {
                if ((submenu.Caption == "Left hand" || submenu.Caption == "Right hand") && q.Tag is Item ikonItem)
                {
                    ...
                    {
                        Possibilities = [
                            new ActionPossibility(new CombatAction(q.Owner, ikonItem.Illustration, "Draw Shadow", [Trait.Manipulate, Trait.Basic], "eee", Target.Self())
            ...
            q.ProvideActionIntoPossibilitySection = (q, section) =>
            {
                if (section.PossibilitySectionId == PossibilitySectionId.MainActions && q.Tag is Item ikonItem && q.Owner.HeldItems.Count < 2)
                {
                    ...
                    List<Possibility> list = ((IEnumerable<Possibility>)(from strike in q.Owner.QEffects.Select(delegate (QEffect qf)
                        {
                            ...
                            return combatAction2;
                                                                         ...
                                                                         select new ActionPossibility(strike)
                    )).ToList();
        ...
        {
            if (action.ActionId == ActionId.DrawItem && ikon.IsIkonItem(action.Item))
            {
                action.ActionCost = 0;
            }
        });
